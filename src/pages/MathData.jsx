...
const tools = [
  {
    label: 'Spotify Listening History Parser',
    url: '/spotify-parser',
  },
  {
    label: 'NFL Stats Tracker',
...
export default function MathData() {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-8 pt-16">
      ...
      <h1 className="text-4xl font-bold mb-2 text-slate-300">Math & Data</h1>
      <p className="text-slate-400 mb-10">Calculators, converters & business tools</p>
      ...
      <div className="flex flex-col gap-4 w-full max-w-md mb-10">
        {tools.map((tool) => (
          <a
            key={tool.label}
            href={tool.url ?? '#'}
            target={tool.url && tool.url.startsWith('http') ? '_blank' : undefined}
            rel={tool.url && tool.url.startsWith('http') ? 'noopener noreferrer' : undefined}
            className={`
